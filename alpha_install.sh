...
# версия: 3.3.0 (Оптимизация под РФ + Полный функционал)
# ==============================================================================
...
clear_telemt() {
    ...
    log_step "Перезагрузка systemd" "systemctl daemon-reload"
}
...
print_status() {
    ...
    printf "${L_IND}${BOLD}статус Telemt: %b%s${NC} | ${BOLD}статус Zapret: %b%s${NC}\n" "$c_t" "$s_t" "$c_z" "$s_z"
}
...
EOF
            ...
            5) menu_telemt_users ;;
            6) printf "\n"; if [ ! -f "$T_SERVICE" ]; then echo -e "${L_IND}${BOLD}${C_RED}!! Ошибка: не установлен${NC}"; else journalctl -u telemt -n 50 --no-pager; fi; prompt_user "[Enter]" wait ;;
     ...
     while true; do
        ...
        case "$act" in
            ...
            2)  prompt_user "Имя" nname
                [ -n "$nname" ] && echo "$nname = \"$(openssl rand -hex 16)\"" >> "$T_CONF" && systemctl restart telemt ;;
            3)  mapfile -t U_LIST < <(sed -n '/\[access.users\]/,$p' "$T_CONF" | grep "=" | awk '{print $1}' | sort -u)
                ...
                if [[ "$uidx" =~ ^[0-9]+$ ]] && [ "$uidx" -gt 0 ]; then
                    ...
                    sed -i "/^${target} =/d" "$T_CONF" && systemctl restart telemt; fi ;;
            0) break ;;
        esac
    done
}
...
menu_zapret() {
    while true; do
        ...
        echo -e "${L_IND}${BOLD}${C_SKY}1 - ${NC}${BOLD}${C_MENU}Установить (Стратегия РФ)${NC}"
        ...
        echo -e "${L_IND}${BOLD}${C_SKY}4 - ${NC}${BOLD}${C_MENU}Сменить порт${NC}"
...
EOF
            ...
            2) systemctl start zapret-tpws ;;
            3) systemctl stop zapret-tpws ;;
            4) prompt_user "Новый порт" nport
               [[ "$nport" =~ ^[0-9]+$ ]] && sed -i "s/--port=[0-9]*/--port=$nport/" "$Z_SERVICE" && systemctl daemon-reload && systemctl restart zapret-tpws ;;
            5) journalctl -u zapret-tpws -n 50 --no-pager; prompt_user "[Enter]" wait ;;
            6) clear_zapret; sleep 1 ;;
            0) break ;;
        esac
    done
}
...
menu_script_manager() {
    while true; do
        printf "\033[H\033[J"; draw_header "МЕНЕДЖЕР СКРИПТА"; echo ""; print_status; echo ""
        refresh_update_marker
        echo -e "${L_IND}1 - Обновить скрипт${U_MARKER}"
        echo -e "${L_IND}2 - Удалить Telemt"
        echo -e "${L_IND}3 - Удалить Zapret"
        echo -e "${L_IND}4 - Удалить всё"
        echo -e "${L_IND}0 - Назад"
        prompt_user "Действие" act
        case "$act" in
            1) log_step "Обновление" "curl -sSL $REPO_URL -o $CLI_PATH && chmod +x $CLI_PATH"; exec "$CLI_PATH" ;;
            2) clear_telemt ;;
            3) clear_zapret ;;
            4) clear_telemt; clear_zapret; rm -f $CLI_PATH; exit 0 ;;
            0) break ;;
        esac
    done
}
...
while true; do
    refresh_update_marker
    printf "\033[H\033[J"; draw_header "СТАЛИН-3000 (v$CURRENT_VERSION)"; echo ""; print_status; echo ""
