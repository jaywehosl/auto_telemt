...
# версия: 3.2.0 (Stealth Edition / IPv6 Priority)
# ==============================================================================
...
M_MAIN_1="Менеджер Telemt (IPv6 Priority)"
M_MAIN_2="Менеджер Zapret (Moscow Strategy)"
M_MAIN_3="Менеджер скрипта и Обслуживание"
M_MAIN_0="Выход"
...
clear_telemt() {
    log_step "Остановка Telemt" "systemctl stop telemt"
    log_step "Удаление файлов" "rm -rf $T_CONF_DIR $T_BIN $T_SERVICE"
    systemctl daemon-reload
}

clear_zapret() {
    log_step "Остановка Zapret" "systemctl stop zapret-tpws"
...
menu_telemt() {
    while true; do
        ...
        echo -e "${L_IND}${BOLD}${C_SKY}1 - ${NC}${BOLD}${C_MENU}Установить / переустановить (IPv6 Mode)${NC}"
        ...
        case "$act" in
            1)  printf "\n"; prompt_user "Порт (443)" p_port; p_port=${p_port:-443}
                ...
                log_step "Установка wget и curl" "apt-get install -y curl wget jq tar openssl -qq"
                ...
                # Блок 3: Предварительная загрузка через IPv6
                log_step "Загрузка секретов (IPv6)" "wget -6 -qO $T_CONF_DIR/cache/proxy-secret https://core.telegram.org/getProxySecret"
                log_step "Загрузка конфига DC (IPv6)" "wget -6 -qO $T_CONF_DIR/cache/proxy-multi.conf https://core.telegram.org/getProxyConfig"
                log_step "Загрузка конфига V6 (IPv6)" "wget -6 -qO $T_CONF_DIR/cache/proxy-multi-v6.conf https://core.telegram.org/getProxyConfigV6"
                cp $T_CONF_DIR/cache/proxy-* $T_CONF_DIR/

                # Блок 2: Шаблон telemt.toml с IPv6
                cat <<EOF > $T_CONF
[general]
...
EOF
                # Блок 1: Systemd unit с WorkingDirectory
                cat <<EOF > $T_SERVICE
[Unit]
...
EOF
                log_step "Старт службы" "systemctl daemon-reload && systemctl enable --now telemt"
...
menu_telemt_users() {
     while true; do
        ...
        if [ ! -f "$T_CONF" ]; then echo -e "${L_IND}${BOLD}${C_RED}!! Не установлен${NC}"; break; fi
        echo -e "${L_IND}${BOLD}${C_SKY}1 - ${NC}${BOLD}${C_MENU}Список и ссылки${NC}"
        echo -e "${L_IND}${BOLD}${C_SKY}2 - ${NC}${BOLD}${C_MENU}Добавить пользователя${NC}"
        ...
        case "$act" in
            ...
            2)  prompt_user "Имя" nname
                if [ -n "$nname" ]; then echo "$nname = \"$(openssl rand -hex 16)\"" >> "$T_CONF"
                    log_step "Обновление" "systemctl restart telemt"; sleep 1; fi ;;
            3)  mapfile -t U_LIST < <(sed -n '/\[access.users\]/,$p' "$T_CONF" | grep -E "^[a-zA-Z0-9]" | grep "=" | awk '{print $1}' | sort -u)
                for i in "${!U_LIST[@]}"; do printf "${L_IND}${BOLD}${C_SKY}%d - ${NC}${BOLD}${C_MENU}%s${NC}\n" "$((i+1))" "${U_LIST[$i]}"; done
                prompt_user "Номер для удаления" uidx
                if [[ "$uidx" =~ ^[0-9]+$ ]] && [ "$uidx" -gt 0 ]; then
                    sed -i "/^${U_LIST[$((uidx-1))]} =/d" "$T_CONF"
                    log_step "Удаление" "systemctl restart telemt"; sleep 1; fi ;;
            0) break ;;
        esac
    done
}
...
menu_zapret() {
    while true; do
        ...
        echo -e "${L_IND}${BOLD}${C_SKY}1 - ${NC}${BOLD}${C_MENU}Установить / Обновить (Stealth)${NC}"
        ...
        printf "\n"; prompt_user "Действие" act
        case "$act" in
            1)  prompt_user "Локальный порт (1080)" z_port; z_port=${z_port:-1080}
                log_step "Зависимости" "apt-get update -qq && apt-get install -y build-essential libnetfilter-queue-dev libmnl-dev libcap-dev zlib1g-dev git -qq"
                log_step "Клонирование" "rm -rf $Z_DIR && git clone --depth=1 https://github.com/bol-van/zapret.git $Z_DIR"
                log_step "Сборка" "make -C $Z_DIR"
                # Блок 4: Обновленная стратегия TPWS
                cat <<EOF > $Z_SERVICE
[Unit]
...
WantedBy=multi-user.target
EOF
                log_step "Запуск" "systemctl daemon-reload && systemctl enable --now zapret-tpws"
                prompt_user "Готово, [Enter]" wait ;;
            2) log_step "Запуск" "systemctl start zapret-tpws"; sleep 1 ;;
            3) log_step "Остановка" "systemctl stop zapret-tpws"; sleep 1 ;;
            4) prompt_user "Новый порт" nport
               if [[ "$nport" =~ ^[0-9]+$ ]]; then
            ...
            5) journalctl -u zapret-tpws -n 50 --no-pager; prompt_user "[Enter]" wait ;;
            0) break ;;
        esac
    done
}
...
menu_maintenance() {
    while true; do
        printf "\033[H\033[J"
        draw_header "ОБСЛУЖИВАНИЕ"; echo ""; print_status; echo ""
        refresh_update_marker
        echo -e "${L_IND}${BOLD}${C_SKY}1 - ${NC}${BOLD}${C_MENU}Обновить/Переустановить менеджер${NC}"
        # Блок 5: Починить апстрим
        echo -e "${L_IND}${BOLD}${C_SKY}2 - ${NC}${BOLD}${C_MENU}Починить апстрим Telemt (IPv6 Update)${NC}"
        echo -e "${L_IND}${BOLD}${C_SKY}3 - ${NC}${BOLD}${C_MENU}Удалить только Telemt${NC}"
        echo -e "${L_IND}${BOLD}${C_SKY}4 - ${NC}${BOLD}${C_MENU}Удалить только Zapret${NC}"
        echo -e "${L_IND}${BOLD}${C_SKY}5 - ${NC}${BOLD}${C_MENU}ПОЛНОЕ УДАЛЕНИЕ ВСЕГО${NC}"
        echo -e "${L_IND}${BOLD}${C_SKY}0 - ${NC}${BOLD}${C_MENU}Назад${NC}"
        printf "\n"; prompt_user "Действие" act
        case "$act" in
            1) log_step "Обновление" "curl -sSL -f ${REPO_URL}?v=$(date +%s) -o $CLI_PATH && chmod +x $CLI_PATH"; exec "$CLI_PATH" ;;
            2) printf "\n"; log_step "Обновление кэша (IPv6)" "wget -6 -qO $T_CONF_DIR/cache/proxy-multi.conf https://core.telegram.org/getProxyConfig && wget -6 -qO $T_CONF_DIR/cache/proxy-multi-v6.conf https://core.telegram.org/getProxyConfigV6"
               log_step "Перезапуск Telemt" "systemctl restart telemt"; sleep 1 ;;
            3) clear_telemt; sleep 1 ;;
            4) clear_zapret; sleep 1 ;;
            5) prompt_user "Удалить всё? (y/n)" cf
               if [[ "$cf" =~ ^[Yy]$ ]]; then clear_telemt; clear_zapret; rm -f $CLI_PATH; exit 0; fi ;;
            0) break ;;
        esac
    done
}
