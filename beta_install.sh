...
# Версия: 1.4.3 (Interaction & Logic Overhaul)
# ==============================================================================
...
L_IND="  " # Глобальный отступ (2 пробела)

BOLD=$(tput bold)
NC='\033[0m' 
MAIN_COLOR='\033[38;5;148m' # Рамки и заголовки (салатовый)
ORANGE='\033[1;38;5;214m'  # Все запросы ввода >> (оранжевый)
SKY_BLUE='\033[1;38;5;81m' # Индексы, инфо, шаги (голубой)
GREEN='\033[1;32m'         # Статус: Работает (зеленый)
RED='\033[1;31m'           # Статус: Ошибка / Опасность (красный)
YELLOW='\033[1;33m'        # Статус: Остановлен (желтый)
...
# Отрисовка шапки. Пульс встроен в правую границу ║
draw_header() {
    local title="$1"
    local width=44
...
# Блок статусов (Вызывается в каждом меню)
msg_status() {
    local s1 s2
...
# Единая функция запроса ввода (оранжевая)
msg_prompt() {
    echo -ne "${L_IND}${BOLD}${ORANGE}>> $1: ${NC}"
}

# Лог выполнения шага (голубой)
msg_step() {
    local msg="$1"
    local cmd="$2"
...
show_links() {
    ...
    if [ -z "$LINKS" ] || [ "$LINKS" == "null" ]; then
        msg_error "Данные не получены. Telemt запущен?"
    else
...
submenu_service() {
    while true; do
        ...
        case $sc in
            1)  echo ""
                msg_prompt "порт для Telemt (443)"; read P_PORT; P_PORT=${P_PORT:-443}
                msg_prompt "SNI домен (google.com)"; read P_SNI; P_SNI=${P_SNI:-google.com}
                msg_prompt "имя администратора"; read P_USER; P_USER=${P_USER:-admin}
                echo ""
                msg_step "Зависимости" "export DEBIAN_FRONTEND=noninteractive; apt-get update -qq && apt-get install -y curl jq tar openssl net-tools -qq"
...
EOF
            ...
            2) echo ""; msg_step "Перезапуск" "systemctl restart telemt"; wait_user ;;
            3) echo ""; msg_step "Остановка" "systemctl stop telemt"; wait_user ;;
            0) break ;;
        esac
    done
}
...
submenu_users() {
    while true; do
        ...
        case $sc in
            ...
            2) echo ""; msg_prompt "имя нового пользователя"; read uname
               if [[ -n "$uname" ]]; then
...
submenu_settings() {
    while true; do
        ...
        msg_prompt "действие"; read sc
        case $sc in
            1) echo ""; journalctl -u telemt -n 50 --no-pager; wait_user ;;
            2) echo ""; msg_prompt "новый порт"; read np
               if [[ $np =~ ^[0-9]+$ ]]; then sed -i "s/^port = .*/port = $np/" $CONF_FILE && systemctl restart telemt; msg_ok "Порт изменен"; fi; wait_user ;;
            3) echo ""; msg_prompt "новый SNI"; read ns
...
submenu_zapret() {
    while true; do
        ...
        msg_prompt "действие"; read sc
        case $sc in
            1)  echo ""; msg_step "Зависимости" "export DEBIAN_FRONTEND=noninteractive; apt-get update -qq && apt-get install -y build-essential libnetfilter-queue-dev libmnl-dev libcap-dev zlib1g-dev git -qq"
...
EOF
                ...
                wait_user ;;
            2) echo ""; msg_step "Запуск" "systemctl start zapret-tpws"; wait_user ;;
            3) echo ""; msg_step "Остановка" "systemctl stop zapret-tpws"; wait_user ;;
            4) echo ""; msg_prompt "удалить Zapret? [y/n]"; read confirm
               [[ "$confirm" =~ ^[Yy]$ ]] && echo "" && msg_step "Удаление" "systemctl stop zapret-tpws; rm -rf $ZAPRET_DIR $ZAPRET_SERVICE; systemctl daemon-reload" && wait_user ;;
            0) break ;;
        esac
    done
}
...
submenu_manager() {
    while true; do
        ...
        # Динамический пункт 1
        if [ "$HAS_UPDATE" = true ]; then
            echo -e "${L_IND}${BOLD}${SKY_BLUE} 1 -${NC} ${BOLD}обновить до ${GREEN}$REMOTE_VERSION${NC}"
        else
            echo -e "${L_IND}${BOLD}${SKY_BLUE} 1 -${NC} ${BOLD}переустановить менеджер (v$CURRENT_VERSION)${NC}"
        fi
        ...
        case $sc in
            ...
            2)  echo ""; msg_prompt "удалить Telemt? [y/n]"; read confirm
                [[ "$confirm" =~ ^[Yy]$ ]] && echo "" && msg_step "Очистка" "systemctl stop telemt; rm -rf $CONF_DIR $SERVICE_FILE $BIN_PATH" && wait_user ;;
            3)  echo ""; msg_prompt "ПОЛНАЯ ОЧИСТКА? [y/n]"; read confirm
                if [[ "$confirm" =~ ^[Yy]$ ]]; then
                    ...
                    echo -e "${L_IND}${RED}${BOLD}Система очищена. До свидания.${NC}"; exit 0
                fi ;;
            0) break ;;
        esac
    done
