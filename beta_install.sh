...
# Тексты меню (Восстановлено и зафиксировано)
...
# Скрытие курсора для чистоты анимации
tput civis 
...
# ------------------------------------------------------------------------------
# 2. Модуль отрисовки (Engine)
# ------------------------------------------------------------------------------

# Перерисовка без мерцания (Cursor Repositioning)
reset_cursor() {
    printf "\033[H"
}
...
draw_header() {
    ...
    # Интеграция пульса в правую стенку рамки
...
wait_user() {
    tput cnorm # Показываем курсор для ожидания
    printf "\n${L_IND}${SKY_BLUE}${BOLD}${L_MSG_WAIT_ENTER}${NC}"
    read -r
    tput civis # Снова прячем
}
...
submenu_service() {
    while true; do
        ...
        echo ""; tput cnorm; msg_prompt "действие"; read sc; tput civis
        case $sc in
            1)  echo ""
                msg_prompt "порт (443)"; read P_PORT; P_PORT=${P_PORT:-443}
                msg_prompt "SNI (google.com)"; read P_SNI; P_SNI=${P_SNI:-google.com}
                msg_prompt "админ"; read P_USER; P_USER=${P_USER:-admin}
                echo ""
...
# 5. Главный цикл
# ------------------------------------------------------------------------------
clear # Первичная очистка
while true; do
    check_updates
    reset_cursor # Возврат курсора в начало вместо clear для flicker-free
    ...
    echo ""; tput cnorm # Показываем курсор перед вводом
    msg_prompt "выберите раздел"; read -t 2 mainchoice
    tput civis # Снова прячем курсор
