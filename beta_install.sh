...
# версия: 1.9.6 (prompt fix + spacing adjustment)
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 2. служебный функционал (backend)
# ------------------------------------------------------------------------------

# запуск асинхронного curl для проверки обновлений
...
# обновление визуального маркера наличия новой версии
...
# безопасный и чистый промпт (исправлена ошибка раскраски y/n)
prompt_user() {
    local text="$1"
    # используем прямую замену ANSI без echo -e в промежуточной переменной
    text="${text//y\//${C_GREEN}y${NC}\/}"
    text="${text//\/n/\/${C_RED}n${NC}}"
    ...
    while true; do
        ...
        case "$act" in
            1)  printf "\n"; prompt_user "порт (443)" p_port; p_port=${p_port:-443}
                ...
                prompt_user "имя администратора" p_user; p_user=${p_user:-admin}; printf "\n"
                log_step "кеш пакетов" "apt-get update -qq"
                log_step "базовые утилиты" "apt-get install -y curl jq tar openssl net-tools -qq"
                ...
                log_step "загрузка бинарного файла" "curl -L '$url' | tar -xz && mv telemt $T_BIN && chmod +x $T_BIN"
                mkdir -p $T_CONF_DIR
                cat <<EOF > $T_CONF
[general]
...
EOF
                log_step "запуск службы" "systemctl daemon-reload && systemctl enable telemt && systemctl restart telemt"
        ...
        case "$act" in
            1)  printf "\n"; mapfile -t U_LIST < <(sed -n '/\[access.users\]/,$p' "$T_CONF" | grep "^[a-zA-Z0-9]" | grep "=" | awk '{print $1}' | sort -u)
                ...
                if [[ "$uidx" =~ ^[0-9]+$ ]] && [ "$uidx" -gt 0 ] && [ "$uidx" -le "${#U_LIST[@]}" ]; then
                    ...
                    printf "\n${L_IND}${BOLD}${C_SKY}ключи доступа: $target${NC}\n"
                    for l in $lnk; do echo -e "${L_IND}${BOLD}${C_ORANGE}${l//0.0.0.0/$ip}${NC}"; done
...
menu_settings() {
    while true; do
        ...
        echo -e "${L_IND}${BOLD}${C_SKY}1 - ${NC}${BOLD}${C_ORANGE}системные логи${NC}"
...
menu_zapret() {
    while true; do
        ...
        echo -e "${L_IND}${BOLD}${C_SKY}1 - ${NC}${BOLD}${C_ORANGE}инсталляция Zapret${NC}"
        ...
        printf "\n"; prompt_user "действие" act
        case "$act" in
            1)  printf "\n"; log_step "библиотеки сборки" "apt-get update -qq && apt-get install -y build-essential libnetfilter-queue-dev libmnl-dev libcap-dev zlib1g-dev git -qq"
...
EOF
                ...
                printf "\n"; prompt_user "готово, нажмите [Enter]" wait ;;
            ...
            4) printf "\n"; prompt_user "удалить Zapret? (y/n)" cf
               if [[ "$cf" =~ ^[Yy]$ ]]; then printf "\n"; log_step "отключение" "systemctl stop zapret-tpws && systemctl disable zapret-tpws"
               log_step "очистка данных" "rm -rf $Z_DIR $Z_SERVICE && systemctl daemon-reload"; sleep 1; fi ;;
            0) check_updates_background; break ;;
        esac
    done
}
...
menu_maintenance() {
    while true; do
        ...
        printf "\n"; prompt_user "выберите действие" act
        case "$act" in
            1) printf "\n"; log_step "загрузка скрипта" "curl -sSL -f ${REPO_URL}?v=$(date +%s) -o $CLI_PATH && chmod +x $CLI_PATH"; sleep 1; exec "$CLI_PATH" ;;
            ...
            3) printf "\n"; prompt_user "ПОЛНОЕ УДАЛЕНИЕ СИСТЕМЫ? (y/n)" cf
               if [[ "$cf" =~ ^[Yy]$ ]]; then printf "\n"; systemctl stop telemt zapret-tpws 2>/dev/null
               systemctl disable telemt zapret-tpws 2>/dev/null; rm -rf "$T_CONF_DIR" "$T_BIN" "$T_SERVICE" "$Z_DIR" "$Z_SERVICE" "$CLI_PATH"
               systemctl daemon-reload; clear; echo -e "${L_IND}${BOLD}${C_RED}все сервисы и скрипт удалены.${NC}"; exit 0; fi ;;
            0) check_updates_background; break ;;
        esac
    done
}
